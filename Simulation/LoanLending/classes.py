...
class PredictiveModel:
    ...
    def computeThresholds(self, env):
        ...
        # the return value of dlo.get_thresholds is (thresh_eqopp, thresh_maxprof)
    ...
    def computeSharedThreshold(self, env):
        # given the env, compute a shared threshold (no difference between groups) using maxutil agent
        pis = np.vstack([env.groups[0].getDistribution(), env.groups[1].getDistribution()])
